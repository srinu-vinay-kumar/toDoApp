...
const sideBarData = ["FreeLance Project", "SBI Outsource", "HPCL Project 1"];
function Sidebar() {
  return (
    <div
      className="w-[240px] h-screen flex flex-col font-Nunito"
      style={{ boxShadow: "0px 0px 8px 0px rgba(54, 89, 226, 0.16)" }}
    >
      <div className=" w-auto h-[60px] flex items-center justify-center gap-[10px] text-[18px] border-b-2 border-b-Foundation">
        <img src={logo} alt="logo" />
        <h1 className=" font-bold">Task Boards</h1>
      </div>
      <div className="flex flex-col px-[10px] py-[24px] gap-[10px] border-b-2 border-b-Foundation justify-start">
        {sideBarData.map((ele) => {
          return (
            <div className=" cursor-pointer h-[44px] w-[220px] flex justify-center align-baseline px-[24px] py-[10px] rounded-md hover:bg-[#EBEEFC] transition-transform duration-500">
              {ele}
            </div>
          );
        })}
      </div>
      <div className="w-auto h-[32px] flex items-center gap-[10px] px-[24px] py-[10px]">
        <svg
        ...
        <h1 className=" text-[#3659E2]">Add new Project</h1>
      </div>
    </div>
  );
}
